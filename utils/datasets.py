...
class SCUTFBP5500Dataset(Dataset):
    ...
    def __init__(self, train=True, transform=None):
        if train:
            self.face_img = pd.read_csv(os.path.join('/home/ubuntu/SCUT5500',
                                                     'train_test_files/split_of_60%training and 40%testing/train.txt'),
                                        sep=' ',names=['image','score'], header=None).iloc[:, 0].tolist()
            self.face_score = pd.read_csv(os.path.join('/home/ubuntu/SCUT5500',
                                                       'train_test_files/split_of_60%training and 40%testing/train.txt'),
                                          sep=' ', names=['image','score'],header=None).iloc[:, 1].astype(np.float).tolist()
        else:
            self.face_img = pd.read_csv(os.path.join('/home/ubuntu/SCUT5500',
...
class SCUTFBP5500Dataset_new():
    ...
    def __init__(self,train=True, transform=None):
        ...
        if train:
              ...
              self.landmarks_frame['score'] = self.landmarks_frame['score'].astype(int) - 1
              
        else:

              self.landmarks_frame = pd.read_csv(os.path.join('/home/ubuntu/SCUT5500',
                                                     'train_test_files/split_of_60%training and 40%testing/test.txt'),sep=' ', names = ['image','score'],header = None)
    ...
    def __getitem__(self, index):
        ...
        image = io.imread('/home/ubuntu/SCUT5500/Images/'+ img_name)
